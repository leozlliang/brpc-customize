...
import com.yy.lite.brpc.utils.ProtoUtils;
 ...
 * 2、http + json
 */
public class HttpRpcClientProtocol extends AbstractProtocol {

    private static final Logger LOG = LoggerFactory.getLogger(HttpRpcClientProtocol.class);
    ...
    public HttpRpcClientProtocol(int protocolType, String encoding) {
...
//            response.setLogId(logId);
            ...
            String[] splits = StringUtils.split(contentTypeAndEncoding, ";");
            int protocolType = HttpRpcClientProtocol.parseProtocolType(splits[0]);
    ...
    public Request decodeRequest(Object packet) {
        try {
            ...
            if (protocolType == ProtocolType.PROTOCOL_HTTP_PROTOBUF_VALUE
                    || protocolType == ProtocolType.PROTOCOL_HTTP_JSON_VALUE) {
                if(httpRequest.getLogId() == -1L){
                    String json = body.toString();
                    JsonObject bodyObject = new JsonParser().parse(json).getAsJsonObject();
                    long logd = bodyObject.get("logId").getAsLong();
                    httpRequest.setLogId(logd);
                }
    ...
    public static int parseProtocolType(String contentType) {
        ...
        if (contentType2.equals(HttpRpcClientProtocol.CONTENT_TYPE_JSON)) {
            return ProtocolType.PROTOCOL_HTTP_JSON_VALUE;
        } else if (contentType2.equals(HttpRpcClientProtocol.CONTENT_TYPE_PROTOBUF)) {
    ...
    public String getContentType(Integer protocolType) {
        String contentType;
        switch (protocolType) {
            case ProtocolType.PROTOCOL_HTTP_JSON_VALUE: {
                contentType = HttpRpcClientProtocol.CONTENT_TYPE_JSON;
                break;
            }
            case ProtocolType.PROTOCOL_HTTP_PROTOBUF_VALUE: {
                contentType = HttpRpcClientProtocol.CONTENT_TYPE_PROTOBUF;
                break;
            }
            default:
...
//                        bodyJson = out.toString(encoding);
                        bodyJson = ProtoUtils.toDefaultJson((Message) body);
                    } else {
    ...
    public Object makeRequest(int id, String methodName, Object[] args) {
        if (protocolType == ProtocolType.PROTOCOL_HTTP_JSON_VALUE) {
            if (args == null || args.length == 0) {
                return null;
            }
            return args[0];
        } else if (protocolType == ProtocolType.PROTOCOL_HTTP_PROTOBUF_VALUE) {
            if (args == null || args.length == 0) {
                return null;
            }
            return args[0];
    ...
    public static void main(String[] args) {
        String sss="{\n" +
                ...
                " \"uid\": 17,\n" +
                " \"subject\":\"math\",\n" +
                " \"questionImage\":\"https://quesgo-india.s3.ap-south-1.amazonaws.com/ques_1571712554017.jpg\"\n" +
                "}";

        JsonObject returnData = new JsonParser().parse(sss).getAsJsonObject();
        System.out.println(returnData.get("logId"));
